# 來源與授權聲明

> 本檔案由素材庫自動維護，供授權稽核使用。

| 項目 | 內容 |
|------|------|
| 套件名稱 | TopDown HD Character Pack — Animated 2D |
| 素材類型 | 角色 |
| 風格 | 像素風 NSFW |
| 授權分類 | 商用需購買授權 |
| 授權條款 | Unity Asset Store EULA |
| 作者 | Unity Asset Store |
| 來源網址 | https://assetstore.unity.com/packages/2d/characters/topdown-hd-character-pack-animated-2d-pixel-characters-282727 |
| 收錄日期 | 2026-07-09 |
| 商用 | 需購買（約 €9.19） |
| 署名 | 依 EULA |
| 備註 | 9 角色 8 方向動畫，含頭像與部分 UI 元件 |

## 授權摘要

- 素材類型：**角色**
- 風格：**像素風 NSFW**
- 授權分類：**商用需購買授權**
- 條款：**Unity Asset Store EULA**

## 原始授權連結

- https://assetstore.unity.com/packages/2d/characters/topdown-hd-character-pack-animated-2d-pixel-characters-282727
